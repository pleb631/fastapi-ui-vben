...
import type { UserListItem, UserListResp, User, UserRoleResp,AllRole } from '#/api';
import {
  reqUserList,
  reqAddOrUpdateUser,
  reqRemoveUser,
...
const handleEditRole = async (id: number) => {
  const result: UserRoleResp = await reqGetUserRole(id);
  editID.value = id;
  visible1.value = true;
  allRole.value = result.all_role;
  userRole.value = result.user_roles;
};

const handleCheckedCitiesChange = async () => {
  if (editID.value) {
    let result: any = await reqUpdateUserRole(editID.value, userRole.value);
    if (result !== null) {
