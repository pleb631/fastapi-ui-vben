  ...
  <div>
    ...
    <el-card style="margin: 10px 0px">
      ...
      <el-table style="margin: 10px 0px" border :data="userArr">
        ...
        <el-table-column label="手机号" align="center" prop="user_phone" show-overflow-tooltip></el-table-column>
        <el-table-column label="邮箱" align="center" prop="user_email" show-overflow-tooltip></el-table-column>
        <el-table-column label="状态" align="center" prop="user_status" show-overflow-tooltip>
          <template #="{ row }">
        ...
        <el-table-column label="操作" width="300px" align="center">
          <template #="{ row }">
            <el-popconfirm :title="`你确定要${!row.user_status ? '启用' : '禁用'}${row.username}?`" width="260px"
              @confirm="updateUserstatus(row)">
              <template #reference>
                <el-button type="primary" size="small" plain :disabled="row.user_type">{{ !row.user_status ? '启用' : '禁用'
                  }}</el-button>
              </template>
            </el-popconfirm>
            <el-popconfirm :title="`你确定要删除${row.username}?`" width="260px" @confirm="deleteUser(row.id)">
              <template #reference>
                <el-button type="danger" size="small" plain :disabled="row.user_type">删除</el-button>
              </template>
            </el-popconfirm>
