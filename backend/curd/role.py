...
async def get_all_role(session: AsyncSession) -> List[Role]:
    role = await session.execute(select(Role.role_name.label("label"), Role.id.label("value")))
    return role.mappings().all()
...
async def get_role_list(
    ...
    role = await session.execute(query.offset((current - 1) * size).limit(size))
